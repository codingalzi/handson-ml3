...
# <div align="center"><img src="https://raw.githubusercontent.com/codingalzi/handson-ml3/master/jupyter-book/imgs/ch04/perceptron.png" width="400"/></div>
# <br>
...
# 회귀 모델의 성능은 **평균 제곱 오차**<font size="2">mean squared error</font>(MSE)로 평가한다.
...
# 훈련셋를 이용한 훈련 과정 중에 가중치 파라미터를 조금씩 반복적으로 조정한다. 
# 이때 비용 함수의 크기를 줄이는 방향으로 조정한다.
# 이를 **경사하강법**<font size="2">gradient descent</font>이라 한다.
# 경사하강법 이해를 위해서는 다음 개념들을 충분히 숙지해야 한다.
...
# 그레이디언트가 가리키는 방향의 **반대 방향**으로 움직이면 빠르게 전역 최솟값에 접근한다.
# 
...
# 
# 단점으로 훈련셋가 크면 그레이디언트를 계산하는 데에 많은 시간과 메모리가 필요해지는 문제가 있다. 
...
# 계산량이 상대적으로 적어 아주 큰 훈련셋를 다룰 수 있으며,
...
# 독립 항등 분포<font size="2">independently and identically distributed</font>(iid)을 따라야 한다.
# 즉, 각각의 샘플이 서로 독립적이어야 하고 또 모든 데이터가 어떤 특성으로 쏠리지 않고 균등하게 분포되어 있어야 한다.
...
# 다항 회귀 모델의 차수에 따라 훈련된 모델이 훈련셋에 과소 또는 과대 적합할 수 있다.
...
# 
# * 과소 적합: 훈련셋와 교차 검증 점수 모두 낮은 경우
# * 과대 적합: 훈련셋에 대한 검증은 우수하지만 교차 검증 점수가 낮은 경우
# 
# 다른 검증 방법은 **학습 곡선**<font size='2'>learning curve</font>을 잘 살펴보는 것이다.
# 학습 곡선은 훈련셋와 검증 세트에 대한 모델 성능을 비교하는 그래프이며,
...
# **과소 적합 모델의 학습 곡선 특징**
# 
# * 훈련셋(빨강)에 대한 성능: 훈련셋가 커지면서 RMSE 증가하지만 
#     훈련셋가 어느 정도 커지면 거의 불변.
# 
