...
# 
# * 과소 적합 모델: 훈련 점수와 교차 검증 점수 모두 낮은 경우.
#     즉, 두 경의 비용<font size='2'>cost</font>이 모두 높은 경우.
# * 과대 적합 모델: 훈련 점수는 높지만 교차 검증 점수가 상대적으로 많이 낮은 경우
#     즉, 훈련셋에 대한 비용은 낮지만 검증셋에 대한 비용은 높은 경우.
# 
...
# 
# 선형 회귀 모델에 대한 **규제**<font size='2'>regularization</font>는 가중치의 영향력을 제한하는 방식으로 이루어진다.
# 즉, 모델의 자유도에 제한을 가하는 형식이며,
...
# :::{admonition} 릿지 회귀의 비용 함수 해석
# :class: info
# 
# 비용 함수의 $J(\theta)$ 전역 최솟값을 찾기 위해서는 $\sum_{i=1}^{n}\theta_i^2$ 값을 최대한 작게 만들어야 하기에
# $\theta_i$의 절대값을 가급적 0에 가깝게 하는 방향으로 경사하강법이 작동한다.
# 이렇게 하면 자연스럽게 모델의 분산도 작아진다.
# 즉, 입력값이 조금 변하더라도 예측값이 변하는 정도가 약해진다.
# 이유는 아래 식에서 볼 수 있듯이 예측값 $\hat y$를 계산할 때 $\theta_i$가 미치는 영향이 작아지기 때문이다. 
# 단, 이는 각 특성의 크기<font size='2'>scale</font>가 일정하다는 전제하에 그렇다.
# 
# $$\hat y = \theta_0 + x_1 \cdot \theta_1 + \cdots + x_n \cdot \theta_{n}$$
# :::

# 아래 그림은 릿지 규제를 적용한 6 가지의 경우를 보여준다.
# 두 경우 모두 규제 강도가 커질 수록 모델의 분산은 줄어들고 편향은 커진다.
# 
...
# * $\alpha$ 와 $\theta_0$: 릿지 회귀의 경우와 동일

# :::{admonition} 라쏘 회귀의 비용 함수 해석
# :class: info
# 
# 비용 함수의 $J(\theta)$ 전역 최솟값을 찾기 위해서는 $\sum_{i=1}^{n}\mid\theta_i\mid$ 값을 최대한 작게 만들어야 하기에
# 별로 중요하지 않은 특성에 대해 $\theta_i$가 0에 빠르게 수렴하도록 훈련 중에 유도된다.
# 이유는 $\mid \theta_i \mid$ 의 미분값이 1또는 -1 이기에 $\theta_i$가 0이 아닌 이상
# 비용 함수의 그레이디언트의 크기가 0에 가까워 지기가 상대적으로 어렵기 때문이다.
# 따라서 경사하강법을 반복적으로 적용하는 훈련 과정동안 중요하지 않은 특성의 가중치를 빠르게 0으로 수렴하게 만들고
# 대신 중요한 특성의 가중치를 결정하는 데에 보다 집중한다.
# :::

# 아래 그림은 라쏘 규제를 적용한 6 가지의 경우를 보여준다.
# 두 경우 모두 규제 강도가 커질 수록 모델의 분산은 줄어들고 편향은 커진다.
# 
# - 왼편: 선형 회귀 모델에 세 개의 $\alpha$ 값 적용.
# - 오른편: 10차 다항 회귀 모델에 세 개의 $\alpha$ 값 적용.
...
# 
# 약간이라도 규제를 사용해야 한다.
# 유용한 속성이 많지 않다고 판단되는 경우엔 중요하지 않은 특성을 무시하게 만드는 라쏘 회귀 또는 엘라스틱 넷을 사용한다.
# 하지만 일반적으로 엘라스틱 넷이 보다 많이 추천된다.
# :::
