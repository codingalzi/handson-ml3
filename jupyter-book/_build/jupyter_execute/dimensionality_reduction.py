...
# **차원의 저주**라 한다.
# 이 문제를 해결하기 위해 특성 수를 (크게) 줄여서 학습 불가능한 문제를 학습 가능한 문제로 만드는 **차원축소** 기법을 사용할 수 있다.
# 차원축소로 인한 정보손실을 어느 정도 감안하면서 훈련 속도와 성능을 최대로 유지하는 것이 주요 목표다.
...
# 차원의 커질 수록 임의의 두 지점 사이의 평균 거리가 매우 멀어진다.
# 이는 특성 수가 아주 많은 경우, 훈련 샘플 사이의 거리가 매우 커서 과대적합 위험도가 커짐을 의미한다.
# 이유는 두 샘플 사이의 거리가 멀어서 기존 값들을 이용한 추정이 여러 과정을 거쳐야 하기 때문에
# 훈련셋에 과하게 의존하게 되기 때문이다.
# 
# 훈련셋의 크기를 키워야 하지만 고차원의 경우 충분히 많은 샘플을 준비하는 일은 사실상 불가능하다는 것이
# **차원의 저주**라는 표현의 핵심이다.
...
# 이것이 차원축소 기법의 핵심이며 크게 사영 기법과 다양체 학습 기법으로 나뉜다.
...
# 근거한다.
# 다양체 가설은 또한 저차원의 다양체 공간으로 차원축소를 진행하면 보다 
# 단순한 모양의 다양체가 된다라는 
# 가설과 함께 사용된다. 
# 
# 하지만 이는 경우에 따라 다르다.
...
# :class: info
# 
# 초평면<font size='2'>hyperplane</font>은 3차원 이상의 고차원에 존재하는 
# 저차원 공간을 가리키며, 아래의 방정식을 만족하는 벡터들의 집합이다. 우리가 상상할 수 있는 평면의 개념과는 다른 모양을 갖는다.
# 
# $$
...
# 단, 데이터셋이 크거나 특성이 많으면 계산에 매우 오랜 시간이 걸릴 수 있다.
...
# X_mmap[:] = X_train  # for 반복문을 이용하여 배치 단위로 저장 가능
# X_mmap.flush()
# 
# # memmap 활용
...
# 
# 희소 행렬을 사용하는 `GaussianRandomProjection` 모델이며 보다 빠르고 메모리 효율적이다. 대용량 데이터셋이 주어진 경우 유용하다.
# 
# ```python
